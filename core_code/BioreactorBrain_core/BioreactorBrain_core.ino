...
LiquidCrystal lcd(2,3,4,5,6,7); //LCD pin name: RS, E, DB4, DB5, DB6, DB7
...
void setup() {
  ...
  // set up the LCD:
  lcd.begin(20, 4);
  lcd.print("BioreactorBrain v1.0");
  ...
  //Check if card is present and can be used:
  if(!SD.begin(10)){
    lcd.setCursor(10,1);
    lcd.print(": X");
  ...
  //Initializing SD card:
  digitalWrite(10, HIGH); //turn ethernet off on the SPI bus
...
//  //Blink read indicator LED:
//  if(probeLedState == LOW) {
//    probeLedState = HIGH;
//    digitalWrite(probeLedPin, probeLedState);
//  } else {
//    probeLedState = LOW;
//    digitalWrite(probeLedPin, probeLedState);
//  }
  ...
  //Heating pad control:
  if(temp < 37){
      digitalWrite(padRelayPin, HIGH); //turn pad to heat water
    }else if (temp >37) {
      digitalWrite(padRelayPin, LOW); //shut pad off when desired temperature is reached
    }
}
